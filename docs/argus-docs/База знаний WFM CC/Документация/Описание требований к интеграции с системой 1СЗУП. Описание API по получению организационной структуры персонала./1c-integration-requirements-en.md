

Table of contents

[Approval sheet	4](#approval-sheet)

[Change registration sheet	5](#change-registration-sheet)

[Terms and definitions	6](#terms-and-definitions)

[1\. integration points	7](#1.-integration-points)

[2\. Concept of problem solving	9](#2.-concept-of-problem-solving)

[1C ZUP database is published on the Web server.	9](#1c-zup-database-is-published-on-the-web-server.)

[3\. description of the exchange rules.	10](#3.-description-of-the-exchange-rules.)

[3.1. data on employees and leave balances retrieved	10](#3.1.-data-on-employees-and-leave-balances-retrieved)

[3.1.1 Employee data	10](#3.1.1-employee-data)

[3.1.2 Data by subdivision	11](#3.1.2-data-by-division)

[3.1.3 Algorithm for calculating vacation balances	11](#3.1.3-algorithm-for-calculating-vacation-balances)

[3.1.4 Exchange with the WFM system	12](#3.1.4-exchange-with-the-wfm-system)

[3.1.4.1 Description of method	13](#3.1.4.1-description-of-method)

[3.2. getNormHours \- Time norm according to the production calendar	16](#3.2.-getnormhours---time-norm-according-to-the-production-calendar)

[3.2.1 Calculation algorithm	16](#3.2.1-algorithm-calculation)

[3.2.2 Exchange with the WFM system	17](#3.2.2-exchange-with-the-wfm-system)

[3.2.2.1 Description of the getNormHours method	17](#3.2.2.1-description-of-the-getnormhours-method)

[3.2.3 Actualization of Time Standards according to the production calendar	20](#3.2.3-actualization-of-time-standards-according-to-the-production-calendar)

[3.2.4 Time rate per week	21](#3.2.4-time-rate-per-week)

[3.3. sendSchedule \- Work schedules	21](#3.3.-sendschedule---work-schedules)

[3.3.1 Calculation algorithm	21](#3.3.1-calculation-algorithm)

[3.3.2 Exchange with the WFM system	22](#3.3.2-exchange-with-the-wfm-system)

[3.3.2.1 Description of the sendSchedule method	23](#3.3.2.1-description-of-the-sendschedule-method)

[3.4. getTimetypeInfo \- timesheet	24](#3.4.-gettimetypeinfo---timesheet)

[3.4.1 Calculation algorithm	24](#3.4.1-calculation-algorithm)

[3.4.2 Exchange with the WFM system	24](#3.4.2-exchange-with-the-wfm-system)

[3.4.2.1 Description of getTimetypeInfo method	25](#3.4.2.1-description-of-gettimetypeinfo-method)

[3.5. sendFactWorkTime \- General description of the exchange algorithm when working with the Operator Work Monitoring Tool (WFM-system)	27](#3.5.-sendfactworktime---general-description-of-the-exchange-algorithm-when-working-with-the-operator-work-monitoring-tool-\(wfm-system\))

[3.5.1 Deviations with time type "RW", "RWN", "NN", "C" (integration point of the timesheet)	27](#3.5.1-deviations-with-time-type-"rw",-"rwn",-"nn",-"c"-\(integration-point-of-the-timesheet\))

[3.5.1.1 Calculation algorithm	28](#3.5.1.1-calculation-algorithm)

[3.5.1.2 Exchange with the WFM system	28](#3.5.1.2-exchange-with-the-wfm-system)

[3.5.1.2.1 Description of the sendFactWorkTime method	28](#3.5.1.2.1-description-of-the-sendfactworktime-method)

[3.5.2 Filling in other fields of created documents	30](#3.5.2-filling-in-other-fields-of-created-documents)

[3.5.3 Conducting documents	31](#3.5.3-conducting-documents)

[4\. Initial data upload	32](#4.-initial-data-upload)

[5\. Necessary improvements to the 1C ZUP configuration	33](#5.-necessary-improvements-to-the-1c-zup-configuration)

[6\. Requirements for setting access rights for the integration system user	34](#6.-requirements-for-setting-up-access-rights-for-the-integration-system-user)

[7\. Requirements for customization of 1C: Salary and Personnel Management	35](#7.-requirements-for-customization-of-1c:-salary-and-personnel-management)

[8\. Appendices	36](#8.-appendices)

[8.1 Appendix 1\. List of 1C \- JSON fields	36](#8.1-appendix-1.-list-of-1c---json-fields)

[8.2 Appendix 2\. Correspondence of time types to 1C ZUP documents	36](#8.2-appendix-2.-correspondence-of-time-types-to-1c-zup-documents)

	

# **Approval sheet**  {#approval-sheet}

| Position | NAME | Caption | Date | Note |
| :---- | :---- | :---- | :---- | :---- |
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |
|  |  |  |  |  |

# **Change registration sheet** {#change-registration-sheet}

| Version | Date | Performer | Changes |
| :---- | :---- | :---- | :---- |
|  |  |  |  |
|  |  |  |  |

# **Terms and definitions** {#terms-and-definitions}

| term | Determination |
| :---- | :---- |
| 1C ZUP, 1C | 1C Salary and Personnel Management CORP configuration, version 3.1 |
| UID | Unique identifier |
| MS. | Information Register |
| RH | Accumulation register |
| Accumulation register | Time rate according to the production calendar |
| Time rate per week | The length of the working week established for an employee |

# **1\. integration points** {#1.-integration-points}

* Work schedules

It is necessary to load into 1C: ZUP 3.1 the work schedules of employees, which can be individualized in the process of planning, linking work schedules to employees

* Vacation schedules

It is necessary to upload from WFM the vacation schedules of operators in Excel format with the following fields filled in:

* Employee \- full name of the employee

* Type of leave \- automatically set to "Basic"

* The start date is the date the vacation begins

* The end date is the date on which the vacation ends

| № | Name | Document |
| :---: | ----- | :---: |
| 1\. | Example of an upload file | ![][image1] |

The file should be uploaded to the 1C system in the document "Vacation Schedule".

* Normative \- employee's time rate according to the production calendar

It is necessary to upload from 1C the normative output of an employee for the year when uploading the employee himself, on request at dismissal

* Vacation balances

It is necessary to be able to upload from 1C the remaining vacation days of an employee as of the requested date

* Timesheets

Planning of work schedules and calculation of actually worked time is performed on the WFM-system side. Accounting of deviations is performed on the side of 1C: ZUP 3.1. Two-way data exchange is required.

The WFM-system is the source of generation of the timesheet.

* The "I", "N" and "B" time types are defined on the 1C side after loading the work schedule from WFM.

* Deviations from the schedule with time type "RV", "RVN", "NV", "C" are initiated in the WFM system. The actual deviation time (date, deviation start time, deviation end time) is transferred from WFM to 1C.

* Determination of the time type is performed on the 1C side based on the deviation of the plan from the fact.

* Depending on the result of determining the type of time, 1C automatically creates the corresponding document.

* Types of time whose input source is 1C: ZUP are listed in [Appendix 2 to the CHTZ](#bookmark=id.p44tylnl4mal).

# **2\. Concept of problem solving** {#2.-concept-of-problem-solving}

## **1C ZUP database is published on the Web server.** {#1c-zup-database-is-published-on-the-web-server.}

This requires:

* Web server installation (Apache or IIS)

* Installing the web server extension on the 1C Platform

An http service is created in the 1C base.

As described below, methods are implemented that generate data packets in JSON format on requests.

**Exchange with the server**

The server must respond to each request with a status (HTTP status) of 200 or 500

* 200 if the request is successfully processed

* 500 if there is an error in the request

The format of the response from the server is JSON.

All access to the server is made via a single address like [http://servername/instancename/hs/apiname.](http://servername/instancename/hs/apiname)

Accesses to the server are formed in the form of POST and GET requests.

# **3\. description of the exchange rules.** {#3.-description-of-the-exchange-rules.}

## **3.1. data on employees and leave balances retrieved** {#3.1.-data-on-employees-and-leave-balances-retrieved}

### **3.1.1 Employee data** {#3.1.1-employee-data}

Data should be uploaded only for employees in the group of subdivisions selected as the exchange subdivision in the register "WFM accounting subdivisions"

Discharge Rules:

* Employees hired are uploaded (Hire Date filled in).

* Terminated employees are uploaded within one month of the termination date (necessary to ensure that WFM receives the termination date). Employees terminated more than one month ago are not included in the upload. When the dismissal date is received in WFM, the employee is deactivated (deactivation occurs the day after the dismissal).

* Data on employees received from the system registers (full name, position, etc.) are unloaded as of the current date (date of sending a request from WFM).

Sources for obtaining employee data:

|  | Data | Data source |
| :---- | :---- | :---- |
| 1 | employee ID | Employees directory |
| 2 | Tab. number | Employees directory |
| 3 | Date of employment | RS Current personnel data of employees |
| 4 | Date of termination | RS Current personnel data of employees |
| 5 | Surname | RS Current personnel data of employees |
| 6 | Name | RS Current personnel data of employees |
| 7 | Patronymic | RS Current personnel data of employees |
| 8 | PID of the post | RS Personnel history of employees |
| 9 | Position | RS Personnel history of employees |
| 10 | Date of change of position | RS Personnel history of employees |
| 10 | Unit ID | RS Personnel history of employees |
| 11 | Number of rates | RS Personnel history of employees |
| 12 | Hours per week | RS Personnel history of employees |
| 13 | Date of norm change per week | RS Personnel history of employees |
| 14 | SN | RS Current personnel data of employees |
| 15 | Db\_ID | RS Current personnel data of employees |
| 16 | area | RS Current personnel data of employees |

### 

### **3.1.2 Data by division** {#3.1.2-data-by-division}

Divisions are unloaded as a two-level hierarchy. The subdivisions are unloaded:

![/download/attachments/276529242/image2023-10-25%2014%3A33%3A12.png?version=1\&modificationDate=1698233595000\&api=v2][image2]

### **3.1.3 Algorithm for calculating vacation balances** {#3.1.3-algorithm-for-calculating-vacation-balances}

Data on employee vacation balances are not stored in 1C objects. Therefore, leave balances for integration purposes are calculated by calculation.

1. Periods of accrual of regular vacation entitlement are determined. This calculation is performed using the standard 1C ZUP algorithm. Currently, the calculation is organized in such a way that each month has only one entitlement accrual date (below is a description of the calculation algorithm from the official ITS site)

2. The number of vacation days for one month (basic vacation and additional vacation, if any) is determined. This calculation is carried out using the standard 1C ZUP algorithm

3. As of the date transferred from WFM of the beginning of the period, the leave balance is calculated. Leave balance is calculated based on leave entitlements and the amount of leave actually used

If the transferred beginning date of the period is less than the beginning of the current month, the leave balance is calculated as of the current month's entitlement accrual date

1. The number of vacation days entitlement is then added to the Vacation Balance on each regular entitlement accrual date

Calculation algorithm of 1C ZUP version higher than 3.1.7

*In version 3.1.7, the algorithm for accruing earned vacation has been refined.*

*It should be recalled that the rules for accrual of earned vacation are determined by [**p. 35 of USSR NKT .04.1930 No. 169:35. 35 of Decree of**](https://its.1c.ru/db/garant/content/81735/1/35) the decree of 30the USSR NKT 30.04.1930 No. 169:35. When calculating the periods of work entitling to proportional additional vacation or vacation compensation upon dismissal, surpluses of less than half a month are excluded from the calculation, and surpluses of at least half a month are rounded up to a full month.*

*The regulation does not define how to calculate half a month. In the previous implementation, the duration of the calendar month in which most of the month worked fell was used to calculate the half-month. But because of the different number of days in calendar months, there were situations when no accrual occurred during a calendar month, or, on the contrary, accrual occurred twice in a calendar month.*

*For example, an employee is hired on the 17th day. Most of the period 17.07.16 \- 16.08.16 falls in August, accordingly, it was considered that the duration of the month is 31 days, half of the month worked at the end of the day 01.08.16. The next period 17.08.16 \- 16.09.16. Most of the period falls in September, accordingly, it was considered that the duration of the month is 30 days, half of the month worked at the end of the day 31.08.16. As a result, in August, the rights were accrued twice.*

*Now the algorithm has been changed:*

* If half a month or more is worked in the calendar month in which the working year begins, we define the vesting date for each calendar month as the end of the previous working month plus half of the month's duration. In this case, we limit the date of accrual of rights to the end of the month. For example, the working year starts on February 15, 17\. The condition "half of the month or more worked" is met, 14 days were worked in February. The dates of rights accrual will be:

  * 28.02.17 (14.02.17 \+ 14 days)

  * 30.03.17 (14.03.17 \+ 16 days)

  * 29.04.17 (14.04.17 \+ 15 days)

  * Etc.

  * If less than half of the calendar month in which the working year begins is worked, the accrual of rights will take place when the sum of the "scraps" equals 15 days. For example, the working year starts on 20.02.17. The dates of rights accrual will be:

    * 06.03.17 (9 days worked in February and 6 in March).

    * 06.04.17 (9 days worked in February and 6 in April).

    * 06.05.17 (9 days worked in February and 6 in May).

    * Etc.

    * The exception is the situation when the working year begins on the 17th day of a month in which there are 31 days. In this case, rights accrual occurs when the sum of the "trims" equals 16 days. For example, the working year starts on 17.01.17. The dates of rights accrual will be:

      * 01.02.17 (15 days worked in January and 1 in February).

      * 01.03.17 (15 days worked in January and 1 in March).

      * 01.04.17 (15 days worked in January and 1 in April).

      * Etc.

Source: ITS website [https://its.1c.ru/db/updinfo\#content:572:1:issogl1\_3](https://its.1c.ru/db/updinfo#content:572:1:issogl1_3)

### **3.1.4 Exchange with the WFM system** {#3.1.4-exchange-with-the-wfm-system}

* On request initiated in the WFM system: method agents

* [When initially uploading data](#bookmark=id.kx0ow0k9oepv)

**Direction of exchange: 1C \-\> WFM**

* The request is automatically initiated by the WFM system once a day (depending on the time set) or at the request of the WFM user.

* This query combines data on departments, employees, and employee leave balances

* The data is uploaded for all employees meeting the selection conditions (selection conditions are described above in the Employee Data section)

* The period passed in the request from WFM is used to retrieve vacation balances

* The data is grouped into arrays: an array of departments, an array of employees with a nested array of employee vacation balances.

### **3.1.4.1 Description of method** {#3.1.4.1-description-of-method}

1\. Input data (WFM-\>1C)

| № | Data | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Start of the period for requesting leave balances | startDate | 2019-01-01 | Date | date |
| 2 | End of leave balances request period | endDate | 2019-12-31 | Date | date |

**Request format from WFM**

Method: **GET**

Template:/agents/{startDate}/{endDate}

2\. Returned data (1C-\>WFM)

| № | Data |  | Name in 1C | Example | Type | Indication that the parameter is mandatory | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Divisions |  | AR\_departments |  | Array | Yes | Array of subdivisions |
| 2 |  | Unit ID | departmentId | 15fcd79e-7352-11e6-80ef-00155d6651df | String | Yes |  |
| 3 |  | Name of division | department | Customer Service Department No. 1 | String | Yes |  |
| 4 |  | UID of the superior unit | parentDepartmentId | b306e82b-74df-11e6-80ef-00155d6651df | String | Yes |  |
| 5 | Employees |  | AR\_agents |  | Array | Yes | Employee data array |
| 6 |  | employee ID | agentId | 7135c70d-b771-11e9-8101-00505690fbe1 |  | Yes |  |
| 7 |  | Job number | tabN | 10-000 | String | Yes |  |
| 8 |  | Surname | lastname | Zubkov | String | Yes |  |
| 9 |  | Name | firstname | Savely | String | Yes |  |
| 10 |  | Patronymic | secondname | Petrovich | String | Yes |  |
| 11 |  | Date of employment | startwork | 2016-09-01 | Date | Yes | date |
| 12 |  | Date of termination | finishwork | 2016-09-01 | Date | No | It's not being sent to a working employee. If fired, the Dismissal Date is not sent from 1c |
| 13 |  | PID of the post | positionId | 79bc9e60-a283-11e7-80f5-00155df19317 | String | Yes |  |
| 14 |  | Job title | position | Leading specialist | String | Yes |  |
| 15 |  | Date of change of position | positionChangeDate | 2016-09-10 | Date | No | date |
| 16 |  | Unit ID | departmentId | 15fcd79e-7352-11e6-80ef-00155d6651df | String | Yes |  |
| 17 |  | Number of rates | rate | 0,5 | number | No |  |
| 18 |  | Login | Login | d6651df | String | No | Additional props |
| 19 |  | Rate per week | normWeek | 40 | number | Yes |  |
| 20 |  | Date of norm change per week | normWeekChangeDate | 2016-09-10 | Date | Yes | If the operator is newly employed, the date of employment is sent. Next, the date of norm change per week and the new norm value in normWeek are sent |
| 21 |  | Additional field 1  | SN | 12345678978 | String | No | Any necessary parameter can be passed to the field |
| 22 |  | Additional ID | Db\_ID | 123647382342 | String | No | It is possible to transfer the ID of another system if necessary |
| 23 |  | Platform | area | Yekaterinburg | String | No | Site. The location of the operator. The field may contain the city of registration or the city from which the employee plans to operate. |
| 24 | Vacation balances |  | AR\_vacation |  | Array | No | Vacation balances data array |
| 25 |  | Date | date | 2019-08-27 | Date | No | Date of accrual of regular vacation entitlement |
| 26 |  | Number of days | vacation | 11.67 | number | No | Accumulated vacation days balance |

**Response Format:**

|  \[   {     { "AR\_departments": \[       {         "departmentId": "03d19161-7352-11e6-80ef-00155d6651df",         { "department": { "service department",         "parent\_departmentId": "03d19161-7352-11e6-80ef-00155d6651df"       },       {         "departmentId": "15fcd79e-7352-11e6-80ef-00155d6651df",         { "department": "Customer service department: customer service group No. 3",         "parent\_departmentId": "03d19161-7352-11e6-80ef-00155d6651df"       }     \],     { "AR\_agents": \[       {         "agentId": "162a53e3-7a51-11e6-80ec-00155d00edab",  "tabN": "223-16",  }, "lastname": "Shastny",  }, "firstname": "Nikita",  }, "secondname": "Alekseevich",         { "startwork": "2016-09-01",         }, "finishwork": null,         "positionId": "79bc9e60-a283-11e7-80f5-00155df19317",         { "position": "Senior Professional",         "positionChangeDate": "2016-09-01",         "departmentId": "b306e82b-74df-11e6-80ef-00155d6651df",         "rate": 1,         "Login": "d6651df",         { "normWeek": 40,         "normWeekChangeDate": "2016-09-01",         "SN": "12345678978",         "Db\_ID": "123647382342",         }, "area": "Yekaterinburg",         { "AR\_vacation": \[           {             "date": "2019-10-16",             { "vacation": 4.67           },           {             "date": "2019-11-15",             { "vacation": 7.00333           }         \]       },       {         "agentId": "ddecd30a-7a52-11e6-80ec-00155d00edab",  "tabN": "224-16",  }, "lastname": "Levine",  }, "firstname": "alina",  }, "secondname": "Vitalievna",         { "startwork": "2016-09-01",         }, "finishwork": null,         "positionId": "79bc9e60-a283-11e7-80f5-00155df19317",         { "position": "Senior Professional",         "positionChangeDate": "2016-09-01",         "departmentId": "b306e82b-74df-11e6-80ef-00155d6651df",         "rate": 1,         { "normWeek": 40,         "normWeekChangeDate": "2016-09-01",         { "AR\_vacation": \[           {  "date": "2019-10-27",             { "vacation": 25.67           },           {  "date": "2019-11-26",             { "vacation": 28.00333           }         \]       }     \]   } \]  |
| :---- |

## **3.2. getNormHours \- Time norm according to the production calendar** {#3.2.-getnormhours---time-norm-according-to-the-production-calendar}

* From the WFM-system will be transmitted [*the weekly time rate*](#bookmark=id.ukvjlujg1t3n) for the employee

* From 1C: Output rate for an employee

### **3.2.1 Algorithm calculation** {#3.2.1-algorithm-calculation}

The time rate is calculated according to the formula:

**Time** *rate* **for the period** \= (Time rate per week / 5 \* Number of working days of a 5-day working week \- Number of hours by which working hours are reduced on the eve of public holidays) \* *Employee rate*

Due to the fact that in 1C ZUP, when setting the weekly norm, the reduction of the norm is immediately taken into account, the multiplication by the rate is not performed when calculating the standard of performance.

**Data sources for calculation in 1C ZUP**

| № | Formula component | Data source | Data |
| :---- | :---- | :---- | :---- |
| 1 | Time rate per week | The parameter is transferred from WFM | normWeek |
| 2 | Number of days of 5-day working week | RS Production calendar data | Sum of days with Day Type Business or Pre-Holiday |
| 3 | Number of hours by which working hours are reduced on the eve of non-working holidays | RS Production calendar data | Sum of days with Day Type Pre-holiday (1 hour each) |
| 4 | Type of Production Calendar for calculation | Reference Guide Employee's work schedule | Production calendar |

**Important: the** type of Production Calendar is set in the employee's main schedule in 1C ZUP (Work Schedules directory). If an employee works according to a regional production calendar, this regional calendar must be specified in 1C in the employee's main schedule.

Reference case

The data is calculated according to the RF Production Calendar 2018\.

|  | Period: 01.01.2018 \- 31.12.2018 |  |  |  |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| Time rate per week | 40 | 40 | 36 | 36 | 30 | 30 |
| Bid | 1 | 0,5 | 1 | 0,25 | 1 | 0,75 |
| Number of calendar working days for the period | 247 | 247 | 247 | 247 | 247 | 247 |
| Number of hours of pre-holiday days for the period | 6 | 6 | 6 | 6 | 6 | 6 |
| Total production rate (hours) | 1970 | 985 | 1772,4 | 886,2 | 1476 | 1107 |

Calculation examples for different periods:

|  | 01.04.2018 \- 31.12.2018 |  |  | 29.06.2018 \- 31.12.2018 |  |  | 03.01.2018 \- 21.05.2018 |  |  |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| Length of the working week | 40 | 36 | 36 | 40 | 36 | 36 | 40 | 36 | 36 |
| Bid | 1 | 1 | 0,75 | 1 | 1 | 0,75 | 1 | 1 | 0,75 |
| Number of calendar working days for the period | 191 | 191 | 191 | 131 | 131 | 131 | 89 | 89 | 89 |
| Number of hours of pre-holiday days for the period | 4 | 4 | 4 | 1 | 1 | 1 | 4 | 4 | 4 |
| Total production rate (hours) | 1524 | 1371,2 | 1028,4 | 1047 | 942,2 | 706,65 | 708 | 636,8 | 477,6 |

### **3.2.2 Exchange with the WFM system** {#3.2.2-exchange-with-the-wfm-system}

* On request initiated in the WFM system: method getNormHours

* [When initially uploading data](#bookmark=id.kx0ow0k9oepv)

**Direction of exchange: 1C \-\> WFM**

* At the time the request is sent to WFM, the Weekly Time Norm must be filled in, as well as the date the weekly norm was changed.

* If the startDate is less than the hire date, the performance standard will be calculated from the hire date to the endDate.

* If the End of Period (endDate) parameter is greater than the Dismissal Date, the standard will be calculated from the StartDate to the Dismissal Date.

* If the startDate is less than the hire date and the endDate is greater than the quit date, the performance standard will be calculated from the Hire Date to the Quit Date.

### **3.2.2.1 Description of the getNormHours method** {#3.2.2.1-description-of-the-getnormhours-method}

1\. Input data (WFM-\>1C)

| № | Data |  | Name in 1C | Example | Type | Obligatory characteristic | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Beginning of the period for calculating the standard of performance |  | startDate | 2019-01-01 | Date | Yes | date |
| 2 | End of the performance standard calculation period |  | endDate | 2019-12-31 | Date | Yes | date |
| 3 | Calculation mode \- month/quarter/year |  | calculationMode | month / quarter / year | String | Yes |  |
| 4 | Employees |  | AR\_agents |  | Array | Yes |  |
| 5 |  | employee ID | agentId | e09df265-7bf4-11e2-9362-001b11b25590 | String | Yes |  |
| 6 |  | Rate per week | AR\_norms |  | Array | Yes |  |
| 7 |  | Time rate per week | normWeek | 36 | number | Yes |  |
| 8 |  | Date of norm change per week | changeDate | 2019-12-31 | Date | No |  |

**Request format from WFM**

Method: **POST**

| Request key | request\_body |
| :---- | :---- |
| getNormHours |  {  "startDate": "2019-01-0",  "endDate": "2019-12-31",  "calculationMode": "quarter" ,  { "AR\_agents":  \[   {     "agentId":"333da0b4-7bff-11e2-9362-001b11b25590", { "AR\_norms" :     \[      {        "normWeek":40,        "changeDate": "2019-03-01"      },      {        "normWeek":20,        "changeDate": "2019-06-01"      },      {        "normWeek":40,        "changeDate": "2019-09-01"      }     \]   },   {     "agentId":"742d89c8-bbfd-11e8-9bce-c809addd7c9c", { "AR\_norms" :     \[      {        "normWeek":36,        "changeDate": "2019-01-01".      }     \]   }  \]}  |

2\. Returned data (1C-\>WFM)

| № | Data | Name in 1C | Example | Type | Obligatory characteristic | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | employee ID | agentId | e09df265-7bf4-11e2-9362-001b11b25590 | String | Yes |  |
| 2 | Time rate according to the production calendar depending on the mode of calculation | normHours | 1970 | number | Yes |  |
| 3 | Settlement start date | startDate | 2018-01-01 | date  | Yes | For the mode: month \- the first day of the month, quarter is the first day of the quarter, year \- the first day of the year, If the hire date is greater than the beginning of the period, the hire date will be passed as the first day of the month/quarter/year |
| 4 | Settlement end date | endDate | 2018-12-31 | date | Yes | For the mode month \- last day of the month quarter year \- the last day of the year If the termination date is less than the end of the period, the termination date will be passed as the last day of the month/quarter/ |

**Response Format:**

| \[{{ "agentId":"f693f3b1-fec4-11ec-80ed-00155d161a16",{ "normHours":454,"startDate":"2023-01-01","endDate":"2023-03-31"},{"agentId":"f693f3b1-fec4-11ec-80ed-00155d161a16",{ "normHours":488,"startDate":"2023-04-01","endDate":"2023-06-30"},{"agentId":"f693f3b1-fec4-11ec-80ed-00155d161a16",{ "normHours":520,"startDate":"2023-07-01","endDate":"2023-09-30"},{"agentId":"f693f3b1-fec4-11ec-80ed-00155d161a16",{ "normHours":511,"startDate":"2023-10-01","endDate":"2023-12-31"}\] |
| :---- |

### **3.2.3 Actualization of Time Standards according to the production calendar** {#3.2.3-actualization-of-time-standards-according-to-the-production-calendar}

*Updating the Time Standards on the production calendar is done:*

1. If the employee's weekly time standard is changed, it is necessary to synchronize the personnel (Agents method) first, so that the WFMCC system receives from 1C the new weekly standard and the date when this standard was changed.

2. When an employee is terminated (to adjust the schedule)

The request is initiated by the WFM user. The query can be passed either for a specific employee or for a group of operators.

### **3.2.4 Time rate per week** {#3.2.4-time-rate-per-week}

The value of the weekly time standard is set in the WFM system for each employee by integration with the 1C system. The 1C system transmits the weekly norm value and the date of the weekly norm change in the Agents method.

The obtained values are passed to 1C as parameters in the getNormHours request \- getting "Production norms by production calendar".

If the employee has just been hired, the 1C system will pass the employee's date of hire in the weekly norm change date.

The WFMCC system will transfer the weekly rate change date and weekly rate received from 1C to the 1C system when a workout is requested.

If during the reporting period there were several normative changes per week \- WFMCC system will send to 1C all dates of normative changes and corresponding normative values per week.

## **3.3. sendSchedule \- Work schedules** {#3.3.-sendschedule---work-schedules}

Employee work schedules are uploaded from the WFM-system. In the 1C ZUP system the document Individual schedule of an employee is created on the basis of the uploaded data.

The Individual Schedule document is generated for a full month for one employee. Accordingly, when uploading data for the year, 12 Individual schedules will be created for one employee.

To unambiguously determine the number of days in a month of the year, schedules are created based on data from the RF Production Calendar.

Possible scenarios for uploading work schedules from WFM to 1C:

* One-time upload of schedules for the year for all employees

* When hiring a new employee

* Updating the employee's schedule to meet the performance standard

* Updating an employee's schedule upon termination of employment

* Changing the work schedule when manually adjusting the schedule in WFM

### **3.3.1 Calculation algorithm** {#3.3.1-calculation-algorithm}

**Accounting of transient shifts.** According to the Customer's ToR: "Night hours are tabulated by a whole shift, for example, if a shift started at 19:00 on 01.02.2018 and ended at 07:00 on 02.02.2018, then the hours are loaded into the timesheet in accordance with the hours worked for 01.02.2018, i.e. if an employee worked the whole shift (12 hours), then the timesheet for 01.02.2018 should contain 12 hours".

Thus, Shift Date \= Start Date of the shift period.

Rules for determining the type of time:

* "H" \- shift entry into the interval 22:00:00-05:59:59

* "I" \- shift entry into the interval 06:00:00-21:59:59

* "B" \- no shift for the day of the month

Subtraction of the duration of unpaid breaks takes place on the WFM side of the system.

### **3.3.2 Exchange with the WFM system** {#3.3.2-exchange-with-the-wfm-system}

* On request initiated in the WFM system: method sendSchedule

**Direction of exchange: WFM \-\> 1C**

* From the WFM system, the dates of work shifts with the number of day and night hours (minus the duration of unpaid breaks) are uploaded.

* The number of hours is unloaded in milliseconds. Conversion to hours takes place on the 1C side.

* If no shift is transferred from WFM on the date of the month, a day off (B) is set in the schedule for that day.

* If an employee's hire date is greater than the start date of the schedule period, then:

  * Schedules for months prior to the month of hire will not be created;

  * The first schedule will be created in the month of hire, with the days from the beginning of the month to the hire date filled with blank values.

  * If an employee has a layoff date less than the end date of the schedule period, then:

    * Schedules for the months following the month of layoff will not be created;

    * The last schedule will be created in the month of layoff, with the days from the layoff date to the month end date filled with blank values.

* If you pass an empty array for an employee (no shifts), no schedules will be created.

* When uploading, the Employee and Schedule Formation Period parameters are checked. If an employee already has an Individual schedule (for a full month) for the period being uploaded, the system makes changes to the existing schedule. If there is no schedule, a new one is created.

* If in 1C there is an individual schedule (in the Carried out state) for an employee for a month that is not full, the schedule from WFM will not be created. An error message will be sent.

* When creating charts, a check is performed to see if the transferred period is included in a closed period (in this case, a closed period is a period earlier than the beginning of the current month). For example, if the current month is September, and the transferred period is January-December, the charts will be created from September to December.

* If the RF production calendar is not loaded in 1C for the transferred period, schedules will not be created. An error message will be sent.

Error messages

| Cause of error | Message to user WFM |
| :---- | :---- |
| The end date of the schedule download period is less than the current month | "It is forbidden to modify schedules for past periods. Individual schedules have not been created." |
| Production calendar not loaded for the period of schedules creation | "For 2021, the RF production calendar has not been filled in\! Individual schedules have not been created." |
| In 1C there is a held individual schedule for not a full month | "For September 2019, there is a part-month schedule for an employee. Delete this schedule in 1C:ZUP and re-upload the schedules from WFM.". |
| No data on shifts | "There is no data for the employee. Individual schedules have not been created." |

### **3.3.2.1 Description of the sendSchedule method** {#3.3.2.1-description-of-the-sendschedule-method}

1\. Input data (WFM-\>1C)

| № | Data |  | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | employee ID |  | agentID | e09df265-7bf4-11e2-9362-001b11b25590 | String |  |
| 2 | Beginning of the charting period |  | period1 | 2018-08-01T00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 3 | End of charting period |  | period2 | 2018-12-31T00:00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 4 | Shift array |  | shift |  | Array | Array of operator shifts in the transferred period |
| 5 |  | Shift start date | date\_start | 2018-08-01T07:00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 6 |  | Number of day shift hours | daily\_hours | 12600000 | number | Duration after subtracting the duration of unpaid breaks. In milliseconds |
| 7 |  | Number of night shift hours | night\_hours | 27000000 | number | Duration after subtracting the duration of unpaid breaks. In milliseconds |

**Request format from WFM**

Method: **POST**

| Request key | request\_body |
| :---- | :---- |
| sendSchedule |  {"agentId":"e09df265-7bf4-11e2-9362-001b11b25590","period1":"2018-08-01T00:00:00Z","period2":"2018-12-31T00:00:00Z","shift":\[{"date\_start":"2018-08-01T19:00:00Z","daily\_hours":"12600000","night\_hours":"27000000"},{"date\_start":"2018-08-03T19:00:00Z","daily\_hours":"12600000","night\_hours":"27000000"},\]}  |

2\. Returned data (1C-\>WFM)

| № | Data | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Result | result | true | String | Result of schedule processing in 1C. The document "Individual work schedule" is executed \- true, not executed \- false. |
| 2 | Message | message | "Individual schedules are created in the 1C:ZUP base" | String | User-friendly description of the result |

**Response Format:**

| { { "result": "true", "message": "Individual schedules have been created in the 1C:ZUP database"}  |
| :---- |

## **3.4. getTimetypeInfo \- timesheet** {#3.4.-gettimetypeinfo---timesheet}

The definition of the time type is performed on the 1C ZUP side.

With the exception of time types "RV", "RVN", "HH", "C" (the rules are described in the previous section), other time types are initiated by creating and executing the corresponding documents by the 1C ZUP user. The list of correspondence of time types and documents is shown in [Appendix 2](#bookmark=id.p44tylnl4mal).

### **3.4.1 Calculation algorithm** {#3.4.1-calculation-algorithm}

Determination of the type of time for each day is made according to the algorithms of 1C ZUP taking into account the preemptive rules.

The rules for displacing one type of time by another depend on the setting of priorities of accrual types in the 1C ZUP system.

For example, if an employee has two types of deviation recorded on the same day: Absenteeism and Sick Leave, time type B (sick leave) will be transferred to WFM. Since sick leave has a higher priority according to the preemptive rules.

### **3.4.2 Exchange with the WFM system** {#3.4.2-exchange-with-the-wfm-system}

* On request initiated in the WFM system: method getTimetypeInfo

**Direction of exchange: 1C \-\> WFM**

* This method is used

  * to generate timesheets in WFM

  * to determine the type of time when a deviation of fact from plan is detected in WFM

  * to generate a report on deviations for the period

* When generating a timesheet in WFM, the WFM system will mass query the operators for a certain period for each day for deviation types. The period of generation is specified by the user in the WFM system.

* To generate the timesheet, the following must be uploaded

  * time type data for each day of the month

  * data for filling in the "No-shows by reason" block

  * data for filling in the "Worked for" block (worked for the first, second half of the month, month)

### **3.4.2.1 Description of getTimetypeInfo method** {#3.4.2.1-description-of-gettimetypeinfo-method}

1\. Input data (WFM-\>1C)

| № | Data |  | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Beginning of the period |  | date\_start | 2018-01-01T00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 2 | End of period |  | date\_end | 2018-12-31T00:00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 3 | Employees |  | AR\_agents |  | Array |  |
| 4 |  | employee ID | agentId | e09df265-7bf4-11e2-9362-001b11b25590 | String | Mandatory parameter |

**Request format from WFM**

Method: **POST**

| Request key | request\_body |
| :---- | :---- |
| getTimetypeInfo |  {{ "date\_start": "2019-02-01T00:00:00","date\_end": "2019-02-28T00:00:00",{ "AR\_agents":\[{"agentId":"333da0b4-7bff-11e2-9362-001b11b25590"},{"agentId":"742d89c8-bbfd-11e8-9bce-c809addd7c9c"},\]}  |

2\. Returned data (1C-\>WFM)

| № | Data |  | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | employee ID |  | agentId | e09df265-7bf4-11e2-9362-001b11b25590 | String |  |
| 2 | Array of dates and time types |  | AR\_date |  | Array | Data for filling in the main block of the timesheet |
| 3 |  | Date | date | 2018-01-01T00:00:00Z | Date | UTC ISO8601 |
| 4 |  | Time type (Letter code) | timetype | Я | String |  |
| 5 |  | Number of hours | hours | 11 | number | If the time type does not have a number of hours, 0 will be transmitted |
| 6 | Data array on no-shows |  | AR\_N |  | Array | Data for completing the block "No-shows by reason" |
| 7 |  | Time type (Letter code) | timetype\_N | HH | String |  |
| 8 |  | Number of days (hours) | hours\_N | 2(6) | String | In the whole-day type, the deviation is only the number of days. For example, OT 14 (14 days of vacation) |
| 9 | Days for the 1st half of the month |  | half1\_days | 5 | number | Data for filling in the "Worked for" block The number of working days and hours is transferred. 1st half of the month \- in any month counts from the 1st to the 15th day inclusive. |
| 10 | Hours for the 1st half of the month |  | half1\_hours | 43 | number |  |
| 11 | Days for the 2nd half of the month |  | half2\_days | 7 | number |  |
| 12 | Hours for the 2nd half of the month |  | half2\_hours | 60 | number |  |

**Response Format:**

| \[    {        "agentId": "8695b47a-7cbe-11e2-9368-001b11b25590",        { "AR\_date": \[            {                "date": "2018-03-27T21:00:00Z",                { "timetype": { "self",                { "hours": 11            },            {                "date": "2018-03-27T21:00:00Z",                { "timetype": "c",                { "hours": 2,75            },            {                "date": "2018-03-28T21:00:00Z", { "timetype": { "en",                "hours": 0            }        \],        { "AR\_N": \[            { }, "timetype\_N": { "timetype\_N",                "hours\_N": "2(5)"            },            { { "timetype\_N": "OT",                "hours\_N": "5"            }        \],        { "half1\_days": 6,        "half1\_hours": 61,        "half2\_days": 7,        "half2\_hours": 68    },    {        "agentId": "333da084-7bff-11e2-9362-001b11b25590",        { "AR\_date": \[            {                "date": "2018-02-28T21:00:00Z",                { "timetype": { "l",                { "hours": 8            }        \],        { "AR\_N": \[\],        { "half1\_days": 9,        "half1\_hours": 71,        "half2\_days": 11,        "half2\_hours": 88    }\] |
| :---- |

## **3.5. sendFactWorkTime \- General description of the exchange algorithm when working with the Operator Work Monitoring Tool (WFM-system)** {#3.5.-sendfactworktime---general-description-of-the-exchange-algorithm-when-working-with-the-operator-work-monitoring-tool-(wfm-system)}

The tool uses two methods of querying 1C ZUP:

1. Request from 1C of registered types of deviations and events according to the planned work schedule. Necessary to get up-to-date information on the availability of deviation documents registered in the 1C system.

**The method is described in the item [Time Sheet](#bookmark=id.uod7md27lci)**

1. Creation of deviation documents in 1C with the type of time RV, RVN, NV, S.

The event occurs when the responsible employee confirms in WFM that the actual time worked deviates from the planned time.

**The method is described under [Deviations with time type "RV", "RVN", "NV", "C"](#bookmark=id.6pbx2ipg3wgf)**

### **3.5.1 Deviations with time type "RW", "RWN", "NN", "C" (integration point of the timesheet)** {#3.5.1-deviations-with-time-type-"rw",-"rwn",-"nn",-"c"-(integration-point-of-the-timesheet)}

Deviations from the schedule with time type "RV", "RVN", "NV", "C" are initiated in the WFM system. The actual deviation time is transferred from WFM to 1C.

Determination of the time type is performed on the 1C side based on the deviation of the plan from the fact.

Depending on the result of determining the type of time, 1C automatically creates the corresponding document:

* "RV", "RVN" \- document Work on holidays and weekends

* "Absence" \- document Absence (sickness, absenteeism, failure to appear) with the reason of absence "Absence for an unexplained reason"

* "C" \- document Overtime work

The document is carried out programmatically at creation. Carrying out in the mode of subsystem Personnel.

If the document was created by mistake, it is necessary to delete it in 1C and upload it from WFM again. Or correct the document in 1C.

### **3.5.1.1 Calculation algorithm** {#3.5.1.1-calculation-algorithm}

The employee's deviation period is uploaded from the WFM system.

Determination of Time Type is based on comparison of actual data transferred from WFM with planned data of the work schedule.

| Plan (hour) | Actual deviation (hour) | Interval entry | Type of time, h |
| :---- | :---- | :---- | :---- |
| 0 | 8 | 06:00 \- 22:00 | RV 8\. |
| 0 | 4 | 22:00 \- 06:00 | WFH 4 |
| 8 | 0 |  | HH 8 |
| 8 | 4 |  | HH 4 |
| 8 | 10 |  | С 2 |

### **3.5.1.2 Exchange with the WFM system** {#3.5.1.2-exchange-with-the-wfm-system}

* On request initiated in the WFM system: method sendFactWorkTime

(the conditions under which the request is initiated are described in the case studies on working with the Operator Control Tool in the WFM system).

**Direction of exchange: WFM \-\> 1C**

* The periods of deviations are reflected by 15 minutes (600000 ms) each

* The conversion to hours takes place on the 1C side.

### **3.5.1.2.1 Description of the sendFactWorkTime method** {#3.5.1.2.1-description-of-the-sendfactworktime-method}

1\. Input data (WFM-\>1C)

| № | Data |  | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | employee ID |  | agentId | e09df265-7bf4-11e2-9362-001b11b25590 | String | Mandatory parameter |
| 2 | Responsible Officer |  | initiator\_name | Ivanov I.I. | String | Initiator of rejection document creation (in 1C will be added to the document comment) |
| 3 | Shift start date |  | date\_start | 2018-12-31T00:00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 4 | Periods in the system |  | loginfo |  | Array |  |
| 5 |  | Date, time of entry | start | 2018-12-31T00:00:00:00Z | Date | ISO8601 format (transmission in UTC, offset or local is possible) |
| 6 |  | Length of time in the system | time | 900000 | number | Data is transmitted for 15 minutes at a time In milliseconds |

**Request format from WFM**

Method: **POST**

| Request key | request\_body |
| :---- | :---- |
| sendFactWorkTime |  {"agentId": "333da0b4-7bff-11e2-9362-001b11b25590","initiator\_name": "Ivanov I.I.""date\_start": "2019-02-01T18:00:00","loginfo":\[{"start":"2019-02-01T18:00:00","time":600000},{"start":"2019-02-01T20:20:00","time":580000}\]}  |

2\. Returned data (1C-\>WFM)

| № | Data | Name in 1C | Example | Type | Commentary |
| :---- | :---- | :---- | :---- | :---- | :---- |
| 1 | Result | result | true | Boolean | In case of successful document creation \- true; in case of errors (document not created) \- false |
| 2 | Name of the created document | doc | No-show | String / null | List of values: Weekend Work, Working Overtime, Failure to appear; If the document is not created, returns null. |

**Response Format:**

With success:

|     {                "result": true,                { "doc": "Weekend Work."            }  |
| :---- |

In error:

| {                "result": false,                "doc": null            } |
| :---- |

### **3.5.2 Filling in other fields of created documents** {#3.5.2-filling-in-other-fields-of-created-documents}

Common Fields:

* Document date \- sets the current date (upload date)

* Document number \- formed automatically by the system when recording

* Organization \- formed on the basis of the employee's current personnel data

* Month \- sets the month of the document date (month of upload)

* Comment \- "Upload from WFM \[Date, time\] \[Name of responsible person from WFM\]"

* Responsible \- the service user "WFM-system" is set.

Document "Work on holidays and weekends"

* Compensation method "Increased payment"

* Payment is made "By the hour."

* The "Employee consent is required", "Consent to work on a weekend and public holiday has been obtained" checkboxes are checked

Document "Absence (sickness, absenteeism, failure to appear)"

* Reason for absence "Unexplained absence"

* If Actual \= 0

  * The "Absence for part of shift" checkbox is not selected

  * If Actual \> 0 but less than Plan

    * The "Absence for part of the shift" checkbox is checked

    * depending on the time of absence, either a Daytime Unexplained Absence or a Nighttime Unexplained Absence (if the period of time of absence includes daytime and nighttime, 2 documents will be created).

Document "Overtime work"

* Compensation method "Increased payment"

* The "Consent for overtime has been obtained" checkboxes are checked

### **3.5.3 Conducting documents** {#3.5.3-conducting-documents}

The documents are executed in the Personnel subsystem mode. The document requires confirmation in the Salary subsystem (signs Calculation approved, Time taken into account is set by the employee of the calculation department).

# **4\. Initial data upload** {#4.-initial-data-upload}

Before you start using the integration, the initial loading of data into the WFM system from 1C ZUP is performed.

Loading is done in stages:

1. The WFM system initiates a query for employees (agents method). This will load the department structure, positions, employee data and leave balances.

2. The WFM system initiates a request for the norm of time according to the production calendar (method [getNormHours](#bookmark=id.8gdje1powo6m)). Beforehand, WFM sets the NormHours per week for employees (if the norm differs from the default 40 hours).

# **5\. Necessary improvements to the 1C ZUP configuration** {#5.-necessary-improvements-to-the-1c-zup-configuration}

The implementation is done through the extensions mechanism (without changing the configuration).

To solve the problem, the following metadata object is created in the extension for the 1C Salary and HR Management CORP configuration:

* HTTP service "wfm\_Energosbyt\_ExchangeWFM"

All methods and business logic are described in the HTTP service module.

# **6\. Requirements for setting up access rights for the integration system user** {#6.-requirements-for-setting-up-access-rights-for-the-integration-system-user}

| Task | Access level |
| :---- | :---- |
| Making changes to the 1C:ZUP configuration | Full rights in the 1C database |
| Installation of the web server extension module to the 1C platform | Administrator rights on the server |
| Web server installation | Administrator rights on the server |
| Publishing HTTP service | Administrator rights in 1C database, Administrator rights on the server |

# **7\. Requirements for customization of 1C: Salary and Personnel Management** {#7.-requirements-for-customization-of-1c:-salary-and-personnel-management}

|  | Integration point with WFM system | Requirements for customization of 1C ZUP |
| :---- | :---- | :---- |
| 1 | Time sheet (all deviations) | Time types of accruals used in deviation documents must be configured in accordance with [Appendix 2](#bookmark=id.p44tylnl4mal) "Matching Time Types to 1C ZUP Documents" |
| 2 | Time sheet (definition of time type) | The rules for displacing time types in the accrual settings (Priority tab) must be consistent with the accounting rules  |
| 3 | Creation of documents with time type "RV", "RVN", "NV", "C" | The accruals and withholdings composition setting must be set: Night watches Overtime Truancy and absenteeism Including intra-shift shifts Work on holidays and weekends |
| 4 | Time rate according to the production calendar | The employee's work schedule (assigned by personnel documents) must be completed: Production calendar for the employee (if the employee works according to the regional production calendar, the corresponding calendar must be set up) Summarized timekeeping  |
| 5 | Employee data | Employees exchanged with WFM must belong to the "Customer Service" division, directory element code "CFR000260". |
| 6 | Work schedules | The RF Production Calendar must be completed for the period of creating individual schedules |
| 7 | Connecting to HTTP service | A user for the WFM system (WFMSystem) must be created in the database. Permissions Full. Without displaying in the selection list. |

# **8\. Appendices** {#8.-appendices}

## **8.1 Appendix 1\. List of 1C \- JSON fields** {#8.1-appendix-1.-list-of-1c---json-fields}

| Data | Name |
| :---- | :---- |
| Employee ID | agentID |
| Tab. number | tabN |
| Date of employment | startwork |
| Date of termination | finishwork |
| Surname | lastname |
| Name | firstname |
| Patronymic | secondname |
| PID of the post | positionID |
| Position | position |
| Unit ID | departmentId |
| Division | department |
| Time rate according to the production calendar | normHours |
| Time rate per week | normWeek |
| Vacation balance | vacation |
| Type of time | timetype |
| Start date | period1 |
| End date | period2 |
| Day | day |
| Date | date |
| Number of hours | hours |
| Bid | rate |

## **8.2 Appendix 2\. Correspondence of time types to 1C ZUP documents** {#8.2-appendix-2.-correspondence-of-time-types-to-1c-zup-documents}

| Designation | Deciphering | Description | Display in the absence control tool in "Event in 1C" | Document in 1C ZUP |
| :---- | :---- | :---- | :---- | :---- |
| PR | Truancy | Absenteeism (absence without a valid excuse for more than 4 consecutive hours or during the entire working day regardless of its continuation) | The WFM system receives from the 1C system through integration, i.e. the event registered in the 1C system must be displayed | Absence (Sickness, absenteeism, failure to appear). Reason for absence \- Absenteeism |
| RP | Simple | Downtime due to employer's fault |  | Employee downtime. Type of downtime \- Through the fault of the employer |
| PC | Professional development | In-service training |  | Absence with pay. Type of absence \- Professional development |
| OT | Vacation | Annual basic paid vacation |  | Vacation |
| OD | Additional vacation | Annual additional paid vacation |  | Vacation. Provide additional vacation. Code of accrual time type \- OD |
| У | Additional leave (paid study leave) | Additional training leave with average earnings for employees combining work and training |  | Vacation. Provide additional vacation. Code of accrual time type \- U |
| UD | Additional leave (unpaid study leave) | Additional leave in connection with training without pay |  | Vacation. Provide additional vacation. Code of accrual time type \- UD |
| Р | Maternity leave | Maternity leave (leave in connection with the adoption of a newborn child) |  | Sick leave. Reason for incapacity for work \- Maternity leave. |
| OW | Parental leave | Leave to care for a child under the age of three years |  | Parental leave |
| DO | Leave without pay with employer's authorization | Leave without pay granted to an employee by the employer's authorization |  | Leave of absence without pay. Type of leave \- Leave without pay in accordance with Part 1 of Article 128 of the Labor Code of the Russian Federation. |
| Б | Sick leave | Temporary disability |  | Sick leave. The reason for the disability is Illness or injury (other than work-related injuries) |
| Т | Unpaid sick leave | Temporary incapacity for work without assignment of benefit in cases provided for by law |  | Time sheet / Absence (sickness, absenteeism, no-show) (note 2 after 1C modification: Payment \- Assign allowance \= False) |
| Г | Fulfillment of public duties | Absences for the period of fulfillment of state or public duties |  | Absence with pay. Type of absence \- Performance of public duties |
| OV | Additional days off (rest days) | Additional days off (paid) |  | Absence with pay. Type of absence \- Additional days off (paid) |
| MO | Medical examination | Absence from work due to medical examination |  | Report Card / Absence with pay. Type of absence \- Time of medical examination |
| CR | Nursing breaks | Absence from the workplace due to nursing breaks |  | Absence with pay. Type of absence \- Breaks for feeding the baby |
| NA | Serving administrative detention | Absence from the workplace due to serving an administrative arrest |  | report card |
| NZ | Suspension of work in case of delayed salary payment | Time of suspension of work in case of delay in payment of wages |  | Absence with pay. Type of absence \- Suspension of work in case of delayed salary payment |
| NB | Suspension without pay | Suspension from work (preventing from work) for reasons stipulated by law, without payroll deduction |  | report card |
| BUT | Suspension with pay | Suspension from work (preventing from work) with payment (allowance) in accordance with the legislation  |  | Absence with pay. Type of absence \- Suspension with pay. |
| GP | Downtime due to employee's fault | Downtime due to employee's fault |  | Employee Downtime. Type of downtime \- Through employee's fault |
| NP | Downtime beyond the control of the employer and employee | Downtime for reasons beyond the control of the employer and the employee |  | Employee downtime. Type of downtime \- Beyond the control of the employer and the employee |
| ZB | Strike | Strike (under the conditions and in the manner prescribed by law) |  | Absence with pay. Type of absence \- Strike |
| NS | Part-time work | Duration of part-time work on the employer's initiative in cases stipulated by the legislation |  | Employee, with a Part-Time Schedule. |
| SP | Time of enforced absenteeism | Time of forced absenteeism in case of recognition of dismissal, transfer to another job or suspension from work as unlawful, with reinstatement to the previous job |  | Absence with pay. Type of Absence \- Time of Involuntary Absence. |
| HR | Reduced working hours in accordance with the law | Reduced working hours against the normal working hours in cases stipulated by legislation |  | Employee type of time schedule of which \- Reduced working hours in accordance with the law |
| DB | Additional leave without pay | Annual additional leave without pay |  | Leave of absence without pay. Type of leave \- Accrual with type of working time DB |
| OZ | Leave without pay in accordance with the law | Leave without pay under conditions stipulated by the current legislation of the Russian Federation |  | Leave of absence without pay. Type of leave \- Leave without pay in accordance with Part 2 of Article 128 of the Labor Code of the Russian Federation |
| UW | Reduced time of on-the-job trainees | Reduced working hours for on-the-job trainees with partial salary retention |  | Employee type of schedule time of which \- On-the-job training time reduction |
| PM | Advanced training in another area | In-service training in another location |  | Absence with pay. Type of absence \- Professional development in another area |
| К | Business trip | Business trip |  | Business trip |
| VM | Shift | Duration of shift work |  | Employee whose type of schedule time is shift work |

[image1]: <data:image/png;base64,>

[image2]: <data:image/png;base64,>
#!/usr/bin/env python3
"""
Minimal database schema for WFM Enterprise Demo
Creates only the essential tables needed for the 10 core endpoints
"""
...
DEMO_SCHEMA = """
-- ============================================================================
-- WFM Enterprise Demo Schema - Essential Tables Only
-- ============================================================================
...
-- Function to get or create demo organization
CREATE OR REPLACE FUNCTION get_demo_organization() RETURNS UUID AS $$
DECLARE
    org_id UUID;
BEGIN
    SELECT id INTO org_id FROM organizations WHERE code = 'DEMO_ORG';
    IF org_id IS NULL THEN
        INSERT INTO organizations (name, code) VALUES ('Demo Organization', 'DEMO_ORG') RETURNING id INTO org_id;
    END IF;
    RETURN org_id;
END;
$$ LANGUAGE plpgsql;

-- Function to get or create demo department
CREATE OR REPLACE FUNCTION get_demo_department() RETURNS UUID AS $$
DECLARE
    dept_id UUID;
    org_id UUID;
BEGIN
    SELECT get_demo_organization() INTO org_id;
    SELECT id INTO dept_id FROM departments WHERE code = 'SUPPORT' AND organization_id = org_id;
    IF dept_id IS NULL THEN
        INSERT INTO departments (organization_id, name, code) VALUES (org_id, 'Customer Support', 'SUPPORT') RETURNING id INTO dept_id;
    END IF;
    RETURN dept_id;
END;
$$ LANGUAGE plpgsql;
...
async def create_schema():
    ...
    try:
        async with engine.begin() as conn:
            ...
            await conn.execute(text(DEMO_SCHEMA))
...
async def main():
    ...
    print("\n🎯 Next step: python create_demo_data.py")

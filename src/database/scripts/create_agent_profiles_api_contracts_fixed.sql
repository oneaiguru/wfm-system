-- API Contracts for agent_profiles table (Fixed Version)
-- Comprehensive API documentation with Russian language support

-- First, create api_helper_queries table if it doesn't exist
CREATE TABLE IF NOT EXISTS api_helper_queries (
    ...
    UNIQUE(endpoint_path, http_method, query_name)
);
...
INSERT INTO api_contracts (
    ...
    'SELECT COUNT(*) FROM agent_profiles WHERE department_name IS NOT NULL'
);
...
INSERT INTO api_contracts (
    ...
    'SELECT 1 FROM agent_profiles WHERE id = $1'
);
...
INSERT INTO api_contracts (
    ...
    'SELECT EXISTS(SELECT 1 FROM agents WHERE id = $1) AND NOT EXISTS(SELECT 1 FROM agent_profiles WHERE agent_id = $1)'
);
...
INSERT INTO api_contracts (
    ...
    ARRAY['agent_profiles', 'task_assignments'],
...
INSERT INTO api_contracts (
    ...
    'SELECT COUNT(*) FROM agent_profiles WHERE department_name ILIKE $1'
);
...
'SELECT 
    ...
    ap.created_at,
    jsonb_build_object(
        ''first_name'', COALESCE(a.first_name, ''Имя не найдено''),
        ''last_name'', COALESCE(a.last_name, ''Фамилия не найдена''),
        ''email'', COALESCE(a.email, ''email@example.com'')
    ) as agent_details
FROM agent_profiles ap
...
-- Test data for agent_profiles (Fixed with record_identifier)
...
\echo ''
\echo '======================================'
\echo 'AGENT PROFILES API CONTRACTS SUMMARY'
\echo '======================================'
\echo 'Created tables: api_helper_queries (if not existed)'
...
\echo '- GET /api/v1/agent-profiles (list with filters)'
\echo '- GET /api/v1/agent-profiles/{id} (get by id)' 
\echo '- POST /api/v1/agent-profiles (create new)'
\echo '- PUT /api/v1/agent-profiles/{id} (update)'
\echo '- DELETE /api/v1/agent-profiles/{id} (delete)'
\echo '- GET /api/v1/agent-profiles/by-department/{department} (by dept)'
\echo ''
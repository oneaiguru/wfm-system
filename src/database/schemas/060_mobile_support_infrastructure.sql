-- Schema 060: Mobile Support Infrastructure (Tasks 1-5)
-- Enhanced mobile functionality for WFM Enterprise
-- Author: DATABASE-OPUS Agent
-- Created: 2025-07-14

-- Task 1: Enhanced Mobile Session Management
CREATE TABLE IF NOT EXISTS mobile_sessions (
    session_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    employee_id UUID NOT NULL,
    device_id VARCHAR(255) NOT NULL,
    device_fingerprint JSONB NOT NULL,
    device_type VARCHAR(50) NOT NULL CHECK (device_type IN ('ios', 'android', 'tablet', 'web')),
    ...
    security_level VARCHAR(20) DEFAULT 'standard' CHECK (security_level IN ('basic', 'standard', 'high', 'biometric')),
...
CREATE TABLE IF NOT EXISTS mobile_performance_metrics (
    ...
    session_id UUID REFERENCES mobile_sessions(session_id) ON DELETE CASCADE,
    employee_id UUID,
...
CREATE TABLE IF NOT EXISTS mobile_attendance_checkins (
    ...
    employee_id UUID NOT NULL,
    session_id UUID REFERENCES mobile_sessions(session_id),
...
CREATE INDEX IF NOT EXISTS idx_mobile_sessions_employee_active ON mobile_sessions(employee_id, is_active);
CREATE INDEX IF NOT EXISTS idx_mobile_sessions_device ON mobile_sessions(device_id, device_type);
CREATE INDEX IF NOT EXISTS idx_mobile_sessions_activity ON mobile_sessions(last_activity DESC);

CREATE INDEX IF NOT EXISTS idx_push_queue_status_priority ON push_notification_queue(status, priority, scheduled_for);
CREATE INDEX IF NOT EXISTS idx_push_queue_employee ON push_notification_queue(employee_id, notification_type);
...
CREATE INDEX IF NOT EXISTS idx_offline_sync_employee_status ON mobile_offline_sync(employee_id, sync_status);
...
CREATE INDEX IF NOT EXISTS idx_performance_employee ON mobile_performance_metrics(employee_id, metric_type);

CREATE INDEX IF NOT EXISTS idx_checkins_employee_date ON mobile_attendance_checkins(employee_id, DATE(checkin_timestamp));
...
CREATE INDEX IF NOT EXISTS idx_notification_delivery_employee ON push_notification_delivery(employee_id, delivery_timestamp);

-- Sample data for testing
-- Sample mobile sessions
INSERT INTO mobile_sessions (employee_id, device_id, device_fingerprint, device_type, device_model, os_version, app_version, push_token, security_level, biometric_enabled) VALUES
(gen_random_uuid(), 'DEVICE001', '{"browser": "Safari", "screen": "1179x2556", "timezone": "Europe/Moscow"}', 'ios', 'iPhone 14 Pro', '17.2.1', '2.1.5', 'push_token_ios_001', 'biometric', true),
(gen_random_uuid(), 'DEVICE002', '{"browser": "Chrome", "screen": "1080x2400", "timezone": "Europe/Moscow"}', 'android', 'Samsung Galaxy S23', '14.0', '2.1.5', 'push_token_android_002', 'standard', false),
(gen_random_uuid(), 'DEVICE003', '{"browser": "Safari", "screen": "1620x2160", "timezone": "Europe/Moscow"}', 'tablet', 'iPad Pro 11"', '17.2.1', '2.1.3', 'push_token_ipad_003', 'high', true);
...
INSERT INTO push_notification_queue (employee_id, target_type, notification_type, title, message, title_ru, message_ru, priority, payload, deep_link) VALUES
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 0), 'individual', 'schedule_change', 'Schedule Updated', 'Your shift for tomorrow has been changed to 10:00-19:00', 'Расписание обновлено', 'Ваша смена на завтра изменена на 10:00-19:00', 'high', '{"shift_id": "12345", "new_start": "10:00", "new_end": "19:00"}', 'wfm://schedule/details/12345'),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 1), 'individual', 'break_reminder', 'Break Time', 'Time for your scheduled break', 'Время перерыва', 'Время вашего запланированного перерыва', 'normal', '{"break_duration": 15}', 'wfm://break/start'),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 2), 'individual', 'overtime_alert', 'Overtime Alert', 'You are approaching overtime hours', 'Предупреждение о сверхурочных', 'Вы приближаетесь к сверхурочным часам', 'high', '{"current_hours": 7.5, "limit": 8}', 'wfm://timesheet/current');

-- Sample offline sync records
INSERT INTO mobile_offline_sync (session_id, employee_id, sync_type, sync_status, items_to_sync, items_synced, data_size_kb, connection_type, battery_level) VALUES
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 0), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 0), 'schedule_sync', 'completed', 25, 25, 156, 'wifi', 85),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 1), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 1), 'timesheet_sync', 'in_progress', 12, 8, 89, '4g', 67),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 2), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 2), 'full_sync', 'failed', 150, 0, 0, '3g', 23);

-- Sample performance metrics
INSERT INTO mobile_performance_metrics (session_id, employee_id, metric_type, metric_name, metric_value, metric_unit, additional_data, screen_name) VALUES
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 0), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 0), 'app_launch', 'cold_start', 2.1, 'seconds', '{"memory_usage": 45, "cpu_usage": 12}', 'login'),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 1), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 1), 'api_response', 'schedule_load', 456, 'ms', '{"endpoint": "/api/v1/schedule", "status": 200}', 'schedule'),
((SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 2), (SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 2), 'page_load', 'timesheet_view', 1.8, 'seconds', '{"data_rows": 30, "charts": 3}', 'timesheet');

-- Sample attendance check-ins
INSERT INTO mobile_attendance_checkins (employee_id, session_id, boundary_id, checkin_type, location_latitude, location_longitude, location_accuracy, is_within_boundary, distance_from_center, method, verification_status) VALUES
((SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 0), (SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 0), (SELECT boundary_id FROM geofencing_boundaries LIMIT 1 OFFSET 0), 'check_in', 55.7559, 37.6177, 5.2, true, 15.3, 'gps', 'verified'),
((SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 1), (SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 1), (SELECT boundary_id FROM geofencing_boundaries LIMIT 1 OFFSET 1), 'check_in', 55.8305, 37.6330, 8.1, true, 23.7, 'gps', 'verified'),
((SELECT employee_id FROM mobile_sessions LIMIT 1 OFFSET 2), (SELECT session_id FROM mobile_sessions LIMIT 1 OFFSET 2), (SELECT boundary_id FROM geofencing_boundaries LIMIT 1 OFFSET 2), 'check_out', 55.6038, 37.7202, 12.4, true, 45.2, 'wifi', 'pending');
...
BEGIN
    ...
    WHERE logout_timestamp < CURRENT_TIMESTAMP - INTERVAL '90 days'
       OR (is_active = false AND created_at < CURRENT_TIMESTAMP - INTERVAL '90 days');
...
COMMENT ON TABLE mobile_sessions IS 'Enhanced mobile session management with device fingerprinting and security levels';
COMMENT ON TABLE push_notification_queue IS 'Real-time push notification management system with multi-language support';
COMMENT ON TABLE mobile_offline_sync IS 'Mobile offline synchronization tracking and conflict resolution';
COMMENT ON TABLE geofencing_boundaries IS 'Location-based attendance verification boundaries';
COMMENT ON TABLE mobile_performance_metrics IS 'Mobile app performance monitoring and analytics';
...
SELECT 'Mobile Support Infrastructure (Schema 060) created successfully' as status,
       'Tasks 1-5 completed: Sessions, Push Notifications, Offline Sync, Geofencing, Performance Metrics' as details;